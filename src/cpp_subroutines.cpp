...
// [[Rcpp::export]]
IntegerVector get_h(Rcpp::List pos, IntegerVector ord){
  ...
  for(int i=0; i<n-1; ++i){
    IntegerVector pos_who = pos[ord[i] - 1];

    // Create an unordered set to store the elements of vec1
    std::unordered_set<int> set1(pos_who.begin(), pos_who.end());

    IntegerVector dists(0);

    int min_dist = pos_who.length();

    for(int j = i+1; j<n; ++j){
      IntegerVector pos_anc = pos[ord[j] - 1];
      ...
      int new_dist = pos_who.length() + pos_anc.length() - 2*n_shared;
